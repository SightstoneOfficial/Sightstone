...
namespace agsXMPP.Xml.Xpnet
{
    ...
    public abstract class Encoding
    {
        ...
        /// </summary>
        protected static readonly int[] asciiTypeTable =
        {
                        ...
                        case BT_MINUS:
                            ...
                            if (charMatches(buf, off, '-'))
                            {
                                if ((off += MinBytesPerChar) == end)
                                    throw new PartialTokenException();
                                checkCharMatches(buf, off, '>');
                                token.TokenEnd = off + MinBytesPerChar;
                                return TOK.COMMENT;
                            }
                            break;
                        default:
        ...
        private TOK scanDecl(byte[] buf, int off, int end, Token token)
        {
            ...
            {
                case BT_MINUS:
                    return scanComment(buf, off + MinBytesPerChar, end, token);
                case BT_LSQB:
                    token.TokenEnd = off + MinBytesPerChar;
            ...
            while (off != end)
            {
                switch (byteType(buf, off))
                {
                    case BT_PERCNT:
                        ...
                        switch (byteType(buf, off + MinBytesPerChar))
                        {
                            case BT_S:
                            case BT_CR:
                            case BT_LF:
        ...
        private TOK scanPi(byte[] buf, int off, int end, Token token)
        {
            ...
            while (off != end)
            {
                switch (byteType(buf, off))
                {
                    ...
                    case BT_CR:
                    case BT_LF:
                        var isXml = targetIsXml(buf, target, off);
                        token.NameEnd = off;
                        off += MinBytesPerChar;
                        while (off != end)
                        {
        ...
        {
            /* "CDATA[".length() == 6 */
            if (end - off < 6*MinBytesPerChar)
                throw new PartialTokenException();
            for (var i = 0; i < CDATA.Length; i++, off += MinBytesPerChar)
        ...
        private TOK scanEndTag(byte[] buf, int off, int end, Token token)
        {
            ...
            while (off != end)
            {
                switch (byteType(buf, off))
                {
                    ...
                    case BT_LF:
                        token.NameEnd = off;
                        for (off += MinBytesPerChar; off != end; off += MinBytesPerChar)
                        {
                            switch (byteType(buf, off))
                            {
                                case BT_S:
        ...
        private TOK scanCharRef(byte[] buf, int off, int end, Token token)
        {
            if (off != end)
            {
                ...
                {
                    case 'x':
                        return scanHexCharRef(buf, off + MinBytesPerChar, end, token);
                    case '0':
                    case '1':
                    case '2':
                    case '3':
        ...
        private TOK setRefChar(int num, Token token)
        {
            if (num < 0x10000)
            {
                switch (charTypeTable[num >> 8][num & 0xFF])
                {
                    ...
                    case BT_LEAD4:
                    case BT_MALFORM:
                        throw new InvalidTokenException(token.TokenEnd - MinBytesPerChar);
                }
        ...
        private bool isMagicEntityRef(byte[] buf, int off, int end, Token token)
        {
            ...
            {
                case 'a':
                    if (end - off < MinBytesPerChar*4)
                        break;
                    switch (byteToAscii(buf, off + MinBytesPerChar))
                    {
                        case 'm':
                            if (charMatches(buf, off + MinBytesPerChar*2, 'p')
                                && charMatches(buf, off + MinBytesPerChar*3, ';'))
                            {
                                token.TokenEnd = off + MinBytesPerChar*4;
                                token.RefChar1 = '&';
                                return true;
                            }
                            break;
                        case 'p':
                            if (end - off >= MinBytesPerChar*5
                                && charMatches(buf, off + MinBytesPerChar*2, 'o')
                                && charMatches(buf, off + MinBytesPerChar*3, 's')
                                && charMatches(buf, off + MinBytesPerChar*4, ';'))
                            {
                                token.TokenEnd = off + MinBytesPerChar*5;
                                token.RefChar1 = '\'';
                                return true;
                            }
                            break;
                ...
                case 'l':
                    if (end - off >= MinBytesPerChar*3
                        && charMatches(buf, off + MinBytesPerChar, 't')
                        && charMatches(buf, off + MinBytesPerChar*2, ';'))
                    {
                        token.TokenEnd = off + MinBytesPerChar*3;
                        token.RefChar1 = '<';
                        return true;
                    }
                    break;
                ...
                case 'q':
                    if (end - off >= MinBytesPerChar*5
                        && charMatches(buf, off + MinBytesPerChar, 'u')
                        && charMatches(buf, off + MinBytesPerChar*2, 'o')
                        && charMatches(buf, off + MinBytesPerChar*3, 't')
                        && charMatches(buf, off + MinBytesPerChar*4, ';'))
                    {
                        token.TokenEnd = off + MinBytesPerChar*5;
                        token.RefChar1 = '"';
        ...
        private TOK scanRef(byte[] buf, int off, int end, Token token)
        {
            ...
            switch (byteType(buf, off))
            {
                ...
                case BT_LEAD4:
                    ...
                    break;
                case BT_NUM:
                    return scanCharRef(buf, off + MinBytesPerChar, end, token);
                default:
                    throw new InvalidTokenException(off);
            }
            while (off != end)
                        ...
                        for (;;)
                        {
                            ...
                            switch (t)
                            {
                                ...
                                case BT_AMP:
                                {
                                    ...
                                    scanRef(buf, off + MinBytesPerChar, end, token);
                                ...
                                case BT_S:
                                    if (normalized
                                        && (off == valueStart
                                            || byteToAscii(buf, off) != ' '
                                            || (off + MinBytesPerChar != end
                                                && (byteToAscii(buf, off + MinBytesPerChar) == ' '
                                                    || byteType(buf, off + MinBytesPerChar) == open))))
                                        normalized = false;
                                    off += MinBytesPerChar;
                                    break;
                                case BT_LT:
                ...
                case BT_EXCL:
                    ...
                    switch (byteType(buf, off))
                    {
                        case BT_MINUS:
                            return scanComment(buf, off + MinBytesPerChar, end, token);
                        case BT_LSQB:
                            return scanCdataSection(buf, off + MinBytesPerChar, end, token);
                    }
                    throw new InvalidTokenException(off);
                case BT_QUEST:
                    return scanPi(buf, off + MinBytesPerChar, end, token);
                case BT_SOL:
                    return scanEndTag(buf, off + MinBytesPerChar, end, token);
                default:
                    throw new InvalidTokenException(off);
            }
            /* we have a start-tag */
                ...
                switch (byteType(buf, off))
                {
                    ...
                    case BT_LF:
                        ...
                        for (;;)
                        {
                            ...
                            {
                                case BT_NMSTRT:
                                    return scanAtts(off, buf, off + MinBytesPerChar, end, token);
                                case BT_LEAD2:
                                    if (end - off < 2)
                                        throw new PartialCharException(off);
                                    if (byteType2(buf, off) != BT_NMSTRT)
            ...
            ContentToken token)
        {
            ...
            {
                case BT_LT:
                    return scanLt(buf, off + MinBytesPerChar, end, token);
                case BT_AMP:
                    return scanRef(buf, off + MinBytesPerChar, end, token);
                case BT_CR:
                    off += MinBytesPerChar;
                    if (off == end)
                        throw new ExtensibleTokenException(TOK.DATA_NEWLINE);
        ...
        public TOK tokenizeProlog(byte[] buf, int off, int end, Token token)
        {
            ...
            {
                case BT_QUOT:
                    return scanLit(BT_QUOT, buf, off + MinBytesPerChar, end, token);
                case BT_APOS:
                    return scanLit(BT_APOS, buf, off + MinBytesPerChar, end, token);
                case BT_LT:
                {
                    off += MinBytesPerChar;
                    if (off == end)
                    ...
                    {
                        case BT_EXCL:
                            return scanDecl(buf, off + MinBytesPerChar, end, token);
                        case BT_QUEST:
                            return scanPi(buf, off + MinBytesPerChar, end, token);
                        case BT_NMSTRT:
                        case BT_LEAD2:
                ...
                case BT_LF:
                    ...
                    return TOK.PROLOG_S;
                case BT_PERCNT:
                    return scanPercent(buf, off + MinBytesPerChar, end, token);
                case BT_COMMA:
                    token.TokenEnd = off + MinBytesPerChar;
                    return TOK.COMMA;
                case BT_LSQB:
                ...
                case BT_RSQB:
                    ...
                    if (charMatches(buf, off, ']'))
                    {
                        if (off + MinBytesPerChar == end)
                            throw new PartialTokenException();
                        if (charMatches(buf, off + MinBytesPerChar, '>'))
                        {
                            token.TokenEnd = off + 2*MinBytesPerChar;
                ...
                case BT_NUM:
                    return scanPoundName(buf, off + MinBytesPerChar, end, token);
                case BT_LEAD2:
                    if (end - off < 2)
                        throw new PartialCharException(off);
        ...
        public string getPublicId(byte[] buf, int off, int end)
        {
            ...
            for (; off != end; off += MinBytesPerChar)
            {
                var c = byteToAscii(buf, off);
                switch (byteType(buf, off))
                {
                    case BT_MINUS:
                    case BT_APOS:
        ...
        {
            var len = str.Length;
            if (len*MinBytesPerChar != end - off)
                return false;
            for (var i = 0; i < len; off += MinBytesPerChar, i++)
            {
