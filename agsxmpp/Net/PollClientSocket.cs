...
namespace agsXMPP.Net
{
    ...
    // set this lower on embedded devices because the key generation is slow there
        ...
        /// </summary>
        public int CountKeys { get; set; } = 256;
        ...
        public override void Connect()
        {
            ...
            var m_thread = new Thread(PollThread);
#if !CF
            // useful in debug
        ...
        private string GenerateRandomKey()
        {
            ...
            return Hash.HexToString(buf).ToLower();
        }
        ...
        {
            var prev = GenerateRandomKey();
            m_Keys = new string[CountKeys];

            for (var i = 0; i < CountKeys; i++)
            {
                var hash = Hash.Sha1HashBytes(prev);
        ...
        private void PollThread()
        {
			...
			*/
            ...
            while (m_Connected)
            {
                ...
                {
                    FireOnError(new PollSocketException("unexpected status code " + resp.StatusCode));
                    return;
                }
