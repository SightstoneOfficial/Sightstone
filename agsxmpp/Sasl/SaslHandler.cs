    ...
    internal class SaslHandler : IDisposable
    {
            ...
            if (e is Features)
            {
                    ...
                    if (args.Auto)
                    {
                        ...
                        if (f.Mechanisms != null)
                        {
                            ...
                            else if (m_XmppClient.UseSso && f.Mechanisms.SupportsMechanism(MechanismType.GSSAPI))
                            {
                                ...
                                var kerbPrinc = f.Mechanisms.GetMechanism(MechanismType.GSSAPI).KerberosPrincipal;
                                if (kerbPrinc != null)
                ...
                else if (!m_XmppClient.Binded)
                {
                    if (f.SupportsBind)
                    {
                        ...
                        var bIq = string.IsNullOrEmpty(m_XmppClient.Resource)
                            ? new BindIq(IqType.set)
                            : new BindIq(IqType.set, m_XmppClient.Resource);
        ...
        private void BindResult(object sender, IQ iq, object data)
        {
            ...
            if (iq.Type == IqType.result)
            {
                ...
                var sIq = new SessionIq(IqType.set, new Jid(m_XmppClient.Server));
