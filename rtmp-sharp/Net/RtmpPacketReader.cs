...
namespace RtmpSharp.Net
{
    ...
    internal class RtmpPacketReader
    {
        ...
        private int readChunkSize = DefaultChunkSize;
        ...
        public void ReadLoop()
        {
            try
            {
                while (Continue)
                {
                    ...
                    var remainingMessageLength = packet.Length + (header.Timestamp >= 0xFFFFFF ? 4 : 0) -
        ...
        private RtmpHeader ReadHeader()
        {
            ...
            if (!rtmpHeaders.TryGetValue(chunkStreamId, out previousHeader) &&
                chunkMessageHeaderType != ChunkMessageHeaderType.New)
