...
{
    internal class SerializerObjectFactory
    {
        // load serializable types within this library
        private static readonly Type[] BaseSerializableTypes = typeof (SerializerObjectFactory).Assembly
            .GetTypes()
            .Where(x => x.GetCustomAttributes<SerializedNameAttribute>().Any())
            .ToArray();

        private readonly Dictionary<Type, MethodFactory.ConstructorCall> constructors =
            new Dictionary<Type, MethodFactory.ConstructorCall>();

        private readonly Dictionary<string, string> localToRemoteNames = new Dictionary<string, string>();
        private readonly Dictionary<string, Type> remoteToLocalNames = new Dictionary<string, Type>();
        ...
        public void Register(Type type)
        {
            ...
            if (constructor == null)
                throw new ArgumentException(
                    string.Format("<{0}> does not have any accessible parameterless constructors.", type.FullName),
                    "type");
