...
namespace RtmpSharp.IO
{
    public class SerializationContext
    {
        ...
        private readonly ObjectWrapperFactory objectWrapperFactory;
        private readonly SerializerObjectFactory serializerObjectFactory;
        ...
        {
            return serializerObjectFactory.GetAlias(typeName);
        }

        internal bool CanCreate(string typeName)
        {
            return serializerObjectFactory.CanCreate(typeName);
        }

        internal bool CanCreate(Type type)
        {
            return serializerObjectFactory.CanCreate(type);
        }

        internal object Create(string typeName)
        {
            return serializerObjectFactory.Create(typeName);
        }

        internal object Create(Type type)
        {
            return serializerObjectFactory.Create(type);
        }

        internal ClassDescription GetClassDescription(object obj)
        {
            return objectWrapperFactory.GetClassDescription(obj);
        }

        internal ClassDescription GetClassDescription(Type type, object obj)
        {
            return objectWrapperFactory.GetClassDescription(type, obj);
        }
